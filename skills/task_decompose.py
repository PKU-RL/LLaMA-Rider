...
def skill_search(target, init_items={}):
	...
	def recur(tgt, tgt_num):
		...
		for (k, c, n) in tgt_sub: # item, consume, number
			n_to_acquire = n
			# if require a tool, it can be replaced by a higher level one
			# e.g., k can be 'wooden_pickaxe'
			tool_level, tool_name = None, None
			for tn in TOOL_NAMES:
				if k.endswith(tn):
					tool_name = tn
					tool_level = k[:-len(tool_name)]
					break
			if tool_name is not None:
				possess_tool_name = None
				for i_ in range(TOOL_LEVELS.index(tool_level), len(TOOL_LEVELS)):
					k_ = TOOL_LEVELS[i_]+tool_name
					if k_ in list(possess.keys()) and possess[k_]>=n:
						assert c==1
						possess_tool_name = k_
						break
				if possess_tool_name is not None:
					#print('replace {} with {}'.format(k, possess_tool_name))
					continue
...
def convert_state_to_init_items(last_init_items, last_skill, last_skill_type, last_skill_done, 
	inventory_name, inventory_quantity, nearby_tools):
	nearby_items = {}
	if last_skill_type==2: # craft skills do not lose nearby items
		for i in list(last_init_items.keys()):
			if i=='crafting_table_nearby':
				if int(nearby_tools['table']):
					nearby_items[i] = 1
			elif i=='furnace_nearby':
				if int(nearby_tools['furnace']):
					nearby_items[i] = 1
			elif i.endswith('_nearby'):
				nearby_items[i] = 1
	...
	for n, q in zip(inventory_name, inventory_quantity):
		...
		if n_ in inventory_items:
			...
			inventory_items[n_] = q
	return dict(nearby_items, **inventory_items), inventory_items, nearby_items
