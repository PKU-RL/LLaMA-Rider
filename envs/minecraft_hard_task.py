...
# [2,5] action space
def transform_action_2_5(act, attack=False):
    assert act.ndim == 2  # (1, 3)
    act = act[0]
    act = act.cpu().numpy()
    act1, act2 = act[0], act[1]

    action = [0, 0, 0, 12, 12, 0, 0, 0]  # self.base_env.action_space.no_op()
    #return [0,0,1,18,12, 1,0,0]
    assert act1 < 2
    if act1 == 1:  # jump
        action[2] = 1
    
    assert act2 < 5
    if act2 == 1:  # camera pitch 
        action[3] = 6
    elif act2 == 2:  # camera pitch 
        action[3] = 18
    elif act2 == 3:  # camera yaw 
        action[4] = 10
    elif act2 == 4:  # camera yaw 
        action[4] = 14

    # use or attack
    action[5] = 3 if attack else 1
    return action  # (8)
