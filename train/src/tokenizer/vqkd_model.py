...
from src.tokenizer.base import VisionTransformer
from src.tokenizer.norm_ema_quantizer import NormEMAVectorQuantizer
from src.tokenizer.teacher import clip, get_dino_vit_base
...
class VQKD(nn.Module):
    def __init__(self,
        ...
        if self.teacher_model_type == 'clip':
            ...
            self.teacher_model, _ = teacher.load("ViT-B/16", device='cpu', jit=False)
...
def vqkd_encoder_base_decoder_1x768x12_clip(pretrained=False, pretrained_weight=None, as_tokenzer=False, img_size=224, 
                                            n_code=8192, code_dim=32, teacher_model_type="clip", **kwargs):
    ...
    # teacher settings
    _ = kwargs.pop("teacher_model_type", teacher_model_type)

    teacher_model_type = teacher_model_type if not as_tokenzer else 'None'
    decoder_out_dim = 512
